...
def build_candidate_pool(
    ...
    *,
    ...
    df["team_games_played"] = df["team_games_played"].fillna(df.groupby("team_abbreviation")["games_played"].transform("max"))
    df["availability_rate"] = safe_divide(df["games_played"], df["team_games_played"])
    df["mpg"] = safe_divide(df["minutes"], df["games_played"])
    ...
    watchlist = (
        ~core
        & (df["minutes"] >= watchlist_min_minutes)
        & (df["games_played"] >= min_games)
    )
