...
def _load_reviewed_live(config: Dict[str, Any]) -> LoadedSources:
    ...
    if not isinstance(roster_addenda, list) or not all(
        isinstance(value, str) and value.strip() for value in roster_addenda
    ):
        raise ContractError("sources.roster_addenda must be a list of non-empty paths")
    paths = {
        key: resolve_path(value)
        for key, value in configured.items()
        if key not in {"roster_source_as_of", "roster_addenda"}
    }
    roster_addendum_paths = [resolve_path(value) for value in roster_addenda]
    ...
    roster_frames = [_read_table(paths["roster"])]
    roster_frames.extend(_read_table(path) for path in roster_addendum_paths)
    ...
    source_manifest = {
        ...
        "roster_status": {
            ...
            "addenda": [portable_path(path) for path in roster_addendum_paths],
