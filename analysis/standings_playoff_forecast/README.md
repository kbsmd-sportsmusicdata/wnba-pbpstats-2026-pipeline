...
- **`sportsdataverse` (default)** — the republished ESPN data under `data/raw/sportsdataverse/wnba_2026/`. Reliable on GitHub-hosted runners, but its release cadence has run a couple of weeks behind the season, which silently caps the cutoff in the past (e.g. frozen at 2026-08-01 while games had been played into mid-August).
- **`espn`** — `scripts/fetch_wnba_espn_2026.py` pulls the schedule and team box scores directly from ESPN's public site API into `data/raw/espn/wnba_2026/`, and the build reads that root via `--sportsdataverse-data-root`. This is the freshest source, but **ESPN's site API returns HTTP 403 to GitHub-hosted Actions runners** — it blocks cloud egress IP ranges, and no header change gets around it. So `espn` only works from a machine ESPN will serve: a local checkout or a self-hosted runner. Run it there and either commit the two parquet files or point the forecast at the output root.
